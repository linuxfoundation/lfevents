...
class wpMediaLibraryCategories {
    ...
    public function __construct() {
        ...
        if ( is_admin() ) {
            ...
            add_filter( 'wp_dropdown_cats', array( $this, 'wpmediacategory_dropdown_cats' ), 10, 2 );
            ...
            add_action( 'parse_tax_query', array( $this, 'wpmediacategory_backend_parse_tax_query' ) );
        ...
        if ( 'upload.php' == $pagenow ) {
            ...
            if ( $taxonomy != 'category' ) {
                ...
                $dropdown_options = array(
                    ...
                    'show_option_not_in' =>  '— ' . __( 'Not in a category', 'wp-media-library-categories' ) . ' —',
                    'hide_empty'         => false,
                    'hierarchical'       => true,
                    'orderby'            => 'name',
                    'selected'           => $selected,
     ...
     * @action wp_dropdown_cats
     */
    public function wpmediacategory_dropdown_cats( $output, $parsed_args ) {
        ...
        if ( isset( $parsed_args['show_option_not_in'] ) && $parsed_args['show_option_not_in'] ) {

            $show_option_not_in = $parsed_args['show_option_not_in'];
            $selected = (( isset($parsed_args['selected']) && 'not_in' === strval($parsed_args['selected']) ) || ( isset($parsed_args['name']) && isset($_REQUEST[strval($parsed_args['name'])]) && 'not_in' === $_REQUEST[strval($parsed_args['name'])] )) ? " selected='selected'" : '';
    ...
    public function wpmediacategory_custom_bulk_admin_footer() {
        ...
        if ( $terms && ! is_wp_error( $terms ) ) :
            ...
            echo 'window.addEventListener(\'load\',function(){';
                ...
                echo 'var optgroup1 = document.createElement("optgroup");';
                echo 'optgroup1.setAttribute("id", "wpmediacategory_optgroup1");';
                echo 'optgroup1.setAttribute("label", "' . html_entity_decode( __( 'Categories', 'wp-media-library-categories' ), ENT_QUOTES, 'UTF-8' ) . '");';

                echo 'var optgroup2 = document.createElement("optgroup");';
                echo 'optgroup2.setAttribute("id", "wpmediacategory_optgroup1");';
                echo 'optgroup2.setAttribute("label", "' . html_entity_decode( __( 'Categories', 'wp-media-library-categories' ), ENT_QUOTES, 'UTF-8' ) . '");';

                // add categories
                foreach ( $terms as $term ) {
                    $sTxtAdd = esc_js( __( 'Add', 'wp-media-library-categories' ) . ': ' . $term->name );
                    echo "optgroup1.appendChild(new Option('" . $sTxtAdd . "', 'wpmediacategory_add_" . $term->term_taxonomy_id . "'));";
                    echo "optgroup2.appendChild(new Option('" . $sTxtAdd . "', 'wpmediacategory_add_" . $term->term_taxonomy_id . "'));";
                }
                // remove categories
                foreach ( $terms as $term ) {
                    $sTxtRemove = esc_js( __( 'Remove', 'wp-media-library-categories' ) . ': ' . $term->name );
                    echo "optgroup1.appendChild(new Option('" . $sTxtRemove . "','wpmediacategory_remove_" . $term->term_taxonomy_id . "'));";
                    echo "optgroup2.appendChild(new Option('" . $sTxtRemove . "','wpmediacategory_remove_" . $term->term_taxonomy_id . "'));";
                }
                // remove all categories
                echo "optgroup1.appendChild(new Option('" . esc_js( __( 'Remove all categories', 'wp-media-library-categories' ) ) . "','wpmediacategory_remove_0'));";
                echo "optgroup2.appendChild(new Option('" . esc_js( __( 'Remove all categories', 'wp-media-library-categories' ) ) . "','wpmediacategory_remove_0'));";

                echo 'document.querySelector("select[name=\'action\']").appendChild(optgroup1);';
                echo 'document.querySelector("select[name=\'action2\']").appendChild(optgroup2);';
            echo "});";
            echo "</script>";
     ...
     */
    function wpmediacategory_backend_parse_tax_query( $query ) {
        ...
        foreach ( get_object_taxonomies( 'attachment','names' ) as $taxonomy ) {
            ...
            if ( isset( $query->query[$taxonomy] ) && $query->query[$taxonomy] ) {
                if ( 'not_in' === $query->query[$taxonomy] ) {
        ...
        // default WP category && filter = 'not in'
        if ( isset( $_REQUEST['cat'] ) && 'not_in' === $_REQUEST['cat'] ) {
            $taxonomy    = 'category';
            $terms       = get_terms( $taxonomy, array('fields'=>'ids','get'=>'all') );
            $tax_query[] = array(
                'taxonomy' => $taxonomy,
    ...
    public function load_media_view_scripts() {
        ...
        wp_enqueue_script( 'wpmlcp-media-views', plugins_url( 'js/wpmlcp-media-views-281' . $suffix . '.js', __FILE__ ), array( 'media-views' ), $plugin_version, true );
        ...
        $media_views_l10n = array(
            ...
            'filter_by'                 => __( 'View all categories', 'wp-media-library-categories' ),
            'not_in'                    => '— ' . __( 'Not in a category', 'wp-media-library-categories' ) . ' —',
        ...
        if ( $select2 === '1' ) {
            wp_enqueue_script( 'wpmediacategory-select2', plugins_url( 'js/select2' . $suffix . '.js', __FILE__ ), array( 'jquery' ), '4.1.0-custom', true );
            wp_enqueue_style( 'wpmediacategory-select2', plugins_url( 'css/select2' . $suffix . '.css', __FILE__ ), array(), '4.1.0' );
        }
