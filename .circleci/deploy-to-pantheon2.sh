...
TERMINUS_DOES_MULTIDEV_EXIST()
{
    ...
    PANTHEON_MULTIDEV_LIST="$(terminus multidev:list -n ${TERMINUS_SITE2} --format=list --field=Name)"
...
then
    terminus env:wake -n "$TERMINUS_SITE2.dev"
    terminus build:env:create -n "$TERMINUS_SITE2.dev" "$TERMINUS_ENV" --clone-content --yes --notify="$NOTIFY"
else
    terminus build:env:push -n "$TERMINUS_SITE2.$TERMINUS_ENV" --yes
fi

set +ex
echo 'terminus secrets:set'
terminus secrets:set -n "$TERMINUS_SITE2.$TERMINUS_ENV" token "$GITHUB_TOKEN" --file='github-secrets.json' --clear --skip-if-empty
set -ex

# Cleanup old multidevs
terminus build:env:delete:pr -n "$TERMINUS_SITE2" --yes
