	...
	class Fuerza_Import_Countries {
		...
		private function import_terms( $countries ) {
			...
			foreach ( $countries as $item ) {
				...
				if ( term_exists( $slug, 'lfevent-country' ) ) {
					continue;
				}
				...
				if ( ! $is_continent ) {
					...
					$term_exists    = term_exists( $continent_slug, 'lfevent-country' );

					if ( ! $term_exists ) {
						$continent_term = wp_insert_term( $continent_name, 'lfevent-country', array( 'slug' => $continent_slug ) );
					} else {
				...
				$response = wp_insert_term( $name, 'lfevent-country', $args );
