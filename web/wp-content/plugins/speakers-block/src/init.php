...
function speakers_block_callback( $attributes, $content ) {
	...
	$out = '<section class="speakers-section wp-block-ugb-container alignfull" style="background: linear-gradient(90deg, #6C3483 0%, #2874A6 100%); color: white;"><h3 class="speakers-section-title">Featured Speakers</h3><ul class="speaker-list grid-x">';
	...
	foreach ( $speakers_to_show as $speaker ) {
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
				$out .= '<li id="speaker-' . get_the_id() . '" class="speaker cell small-6 medium-4 xxlarge-3" data-toggler=".open" style="background: linear-gradient(-45deg, #6C3483 0%, #2874A6 100%);">';
				...
				$out .= '			</div>';
				$linkedin = get_post_meta( $post->ID, 'lfes_linkedin', true );
				$twitter = get_post_meta( $post->ID, 'lfes_twitter', true );
				$website = get_post_meta( $post->ID, 'lfes_website', true );
				...
				if ( $linkedin ) {
					$out .= '				<li><a href="' . $linkedin . '"><svg class="social-icon--linkedin" style="fill:white;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M100.28 448H7.4V148.9h92.88zM53.79 108.1C24.09 108.1 0 83.5 0 53.8a53.79 53.79 0 0 1 107.58 0c0 29.7-24.1 54.3-53.79 54.3zM447.9 448h-92.68V302.4c0-34.7-.7-79.2-48.29-79.2-48.29 0-55.69 37.7-55.69 76.7V448h-92.78V148.9h89.08v40.8h1.3c12.4-23.5 42.69-48.3 87.88-48.3 94 0 111.28 61.9 111.28 142.3V448z"/></svg></a></li>';					
				}
				if ( $website ) {
					$out .= '				<li><a href="' . $website . '"><svg class="social-icon--website" style="fill:white;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" style="fill:white;"><path d="M448 80v352c0 26.51-21.49 48-48 48H48c-26.51 0-48-21.49-48-48V80c0-26.51 21.49-48 48-48h352c26.51 0 48 21.49 48 48zm-88 16H248.029c-21.313 0-32.08 25.861-16.971 40.971l31.984 31.987L67.515 364.485c-4.686 4.686-4.686 12.284 0 16.971l31.029 31.029c4.687 4.686 12.285 4.686 16.971 0l195.526-195.526 31.988 31.991C358.058 263.977 384 253.425 384 231.979V120c0-13.255-10.745-24-24-24z"/></svg></a></li>';					
				}
				$out .= '			</ul>';
				$out .= '		</div>';
				$out .= '	</div>';
